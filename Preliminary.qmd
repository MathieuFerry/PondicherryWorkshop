...
-   Download the sample database used during the workshop (available on our Google Drive or from here (INSERT LINK)). There are two files where our data are stored: "sample.csv" and "sample.rdata". These are exactly the same files except that the "sample.rdata" is a format native to R (the software we use) and allows to save more pre-formatting characteristics of the data so it will be this one we will use in our session, but you can start to have a look a it by opening the csv file.
